...
const paymentSchema = z.object({
  ...
  amount: z.coerce.number({invalid_type_error: "Valor deve ser um número."}).min(0, "Valor do pagamento não pode ser negativo."),
...
export function MarkAsPaidDialog({ rental, isOpen, onOpenChange, onSuccess }: MarkAsPaidDialogProps) {
    ...
    try {
      if (data.amount < 0.01) {
          ...
          toast({
              ...
              description: `O aluguel ID ${rental.id} foi marcado como pago (valor R$ 0,00).`,
              variant: 'success',
          });
      } else {
  ...
  const isZeroPayment = remainingValue < 0.01;
                ...
                {!isZeroPayment ? (
                 <>
                    ...
                    />
                     ...
                     <div className="p-4 bg-blue-50 border border-blue-200 rounded-md text-center text-blue-800">
                        <p>O valor pendente deste contrato é R$ 0,00.</p>
                        <p className="font-semibold">Clique abaixo para quitar o contrato e removê-lo da lista de pendências.</p>
                    </div>
                )}
                 <DialogFooter>
                    <DialogClose asChild>
                    ...
                    <Button type="submit" disabled={isLoading || (!isZeroPayment && form.getValues('amount') <= 0)}>
                        {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : 
                            (isZeroPayment ? <CheckSquare className="mr-2 h-4 w-4"/> : null)
                        }
                        {isZeroPayment ? "Quitar Contrato (R$ 0,00)" : "Registrar Pagamento"}
                    </Button>
                </DialogFooter>
            </form>
        </Form>
