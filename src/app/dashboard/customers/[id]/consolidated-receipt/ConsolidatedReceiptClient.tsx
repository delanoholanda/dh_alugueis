...
import ConsolidatedPrintActions from './ConsolidatedPrintActions';
...
export default function ConsolidatedReceiptClient({
  rentals,
  customer,
  companySettings,
  inventory,
  ...
  return (
    <div className="bg-gray-100 min-h-screen py-8 px-4 print:bg-white print:py-0 print:px-0">
      ...
      <div className="contract-container">
        <ConsolidatedPrintActions customerId={customer.id} customerName={customer.name} />

        <header className="contract-header">
          <div className="logo-container">
            <Image src={displayContractLogo} alt={`${companySettings.companyName} Logo`} fill style={{ objectFit: 'contain' }} priority key={displayContractLogo} data-ai-hint="company logo" />
          </div>
                ...
                <tbody>
                    {rental.equipment.map((eq, itemIndex) => {
                        ...
                        return (
                            <tr key={`${rental.id}-${itemIndex}`}>
                                ...
                                <td className="text-right">{formatToBRL(itemSubtotal)}</td>
                            </tr>
                        );
                    })}
                    ...
                    {typeof rental.discountValue === 'number' && rental.discountValue > 0 && (
                      <tr className="text-orange-600">
                          <td colSpan={4}>Desconto Concedido</td>
        ...
        <div className="contract-summary-grid contract-section mt-4">
          <div>
            <h3 className="font-semibold text-sm mb-1">Termos e Condições:</h3>
            <p className="terms-conditions">{companySettings.contractTermsAndConditions || ''}</p>
          ...
          <div>
            ...
            {!pixPayload && companySettings.pixKey && totalValue > 0 && (
                <div className="pix-section">
